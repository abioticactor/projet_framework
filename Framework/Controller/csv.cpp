...
std::vector<std::shared_ptr<Enseignant>> CSV::importerEnseignants() {
    ...
    if (data.empty() || data[0].size() < 3) return enseignants;
    for (size_t i = 1; i < data.size(); ++i) {
        if(data[i].size() >= 3) {
            enseignants.push_back(std::make_shared<Enseignant>(data[i][1],data[i][2], std::vector<QString>()));
        }
        else {
...
std::vector<std::shared_ptr<Etudiant>> CSV::importerEtudiants() {
    ...
    for (size_t i = 1; i < data.size(); ++i) {
        if(data[i].size() >= 6) {
            etudiants.push_back(std::make_shared<Etudiant>(data[i][1],data[i][2], data[i][3],  std::vector<QString>()));
        }
        else {
